...
import {
  ...
  "@axelar-network/axelar-gmp-sdk-solidity/contracts/libs/Bytes32String.sol";
...
abstract contract DepositServiceBase is IDepositServiceBase {
  ...
  constructor(address gateway_, string memory wrappedSymbol_) {
    ...
    gateway = gateway_;
    wrappedTokenAddress = wrappedTokenEnabled
      ? IAxelarGateway(gateway_).tokenAddresses(wrappedSymbol_)
      : address(0);
    wrappedSymbolBytes =
      wrappedTokenEnabled ? wrappedSymbol_.toBytes32() : bytes32(0);

    // Wrapped token symbol param is optional
    // When specified we are checking if token exists in the gateway
    if (wrappedTokenEnabled && wrappedTokenAddress == address(0)) {

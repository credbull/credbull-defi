...
contract AxelarAuthWeighted is Ownable, IAxelarAuthWeighted {
  ...
  function _transferOperatorship(
    ...
    if (
      operatorsLength == 0 || !_isSortedAscAndContainsNoDuplicate(newOperators)
    ) revert InvalidOperators();
  ...
  function _validateSignatures(
    ...
    for (uint256 i; i < signaturesLength; ++i) {
      ...
      for (
        ;
        operatorIndex < operatorsLength && signer != operators[operatorIndex];
        ++operatorIndex
      ) { }

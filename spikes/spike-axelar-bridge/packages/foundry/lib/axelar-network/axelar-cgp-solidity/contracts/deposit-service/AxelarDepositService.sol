...
contract AxelarDepositService is
  Upgradable,
  DepositServiceBase,
  IAxelarDepositService
{
  ...
  constructor(
    ...
    receiverImplementation =
      address(new ReceiverImplementation(gateway_, wrappedSymbol_));
  }
  ...
  function sendNative(
    ...
    IAxelarGateway(gateway).sendToken(
      destinationChain, destinationAddress, wrappedSymbol(), amount
    );
  }
  ...
  function addressForNativeUnwrap(
    ...
    return _depositAddress(
      salt,
      abi.encodeWithSelector(
        ReceiverImplementation.receiveAndUnwrapNative.selector,
        refundAddress,
        recipient
      ),
      refundAddress
  ...
  function refundTokenDeposit(
    ...
    for (uint256 i; i < tokensLength; ++i) {
      ...
      if (refundTokens[i] == intendedToken && msg.sender != refundAddress) {
        continue;
      }
  ...
  function refundNativeDeposit(
    ...
    if (
      addressForNativeDeposit(
        salt, refundAddress, destinationChain, destinationAddress
      ).balance > 0 && msg.sender != refundAddress
    ) {
      return;
    }
  ...
  function refundNativeUnwrap(
    ...
    for (uint256 i; i < tokensLength; ++i) {
      ...
      if (refundTokens[i] == wrappedTokenAddress && msg.sender != refundAddress)
      {
        continue;
      }
            ...
            abi.encodePacked(
              ...
              keccak256(
                abi.encodePacked(
                  ...
                  abi.encode(delegateData, refundAddress)
                )
              )
            )
          )

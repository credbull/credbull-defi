...
import {
  ...
  WeightedSigners,
  Proof
} from "../types/WeightedMultisigTypes.sol";
...
abstract contract BaseWeightedMultisig is IBaseWeightedMultisig {
  ...
  function _validateProof(
    ...
    if (
      signerEpoch == 0 || currentEpoch - signerEpoch > previousSignersRetention
  ...
  function _validateSignatures(
    ...
    for (uint256 i; i < signaturesLength; ++i) {
      ...
      for (
        ;
        signerIndex < signersLength
          && recoveredSigner != signers[signerIndex].signer;
        ++signerIndex
      ) { }
   ...
   */
  function _baseWeightedMultisigStorage()
    private
    pure
    returns (BaseWeightedMultisigStorage storage slot)
  {
    assembly {

...
import { IAxelarValuedExpressExecutable } from
  "../interfaces/IAxelarValuedExpressExecutable.sol";
import {
  SafeTokenTransferFrom, SafeTokenTransfer
} from "../libs/SafeTransfer.sol";
  ...
  IAxelarValuedExpressExecutable
{
  ...
  function execute(
    ...
    address expressExecutor =
      _popExpressExecutor(commandId, sourceChain, sourceAddress, payloadHash);
    ...
    emit ExpressExecutionFulfilled(
      commandId, sourceChain, sourceAddress, payloadHash, expressExecutor
    );

    {
      (address tokenAddress, uint256 value) =
        contractCallValue(sourceChain, sourceAddress, payload);
  ...
  function expressExecuteWithToken(
...

    {
      ...
      IERC20(gatewayToken).safeTransferFrom(
        expressExecutor, address(this), amount
      );
    }
  ...
  function _transferFromExecutor(
    ...
    if (tokenAddress == address(0)) {
      ...
      IERC20(tokenAddress).safeTransferFrom(
        expressExecutor, address(this), value
      );
    }
  }

...
import { IInterchainGasEstimation } from
  "../interfaces/IInterchainGasEstimation.sol";
...
abstract contract InterchainGasEstimation is IInterchainGasEstimation {
  ...
  ) external view returns (GasInfo memory) {
   ...
   */
  function _setGasInfo(
    string calldata chain,
    GasInfo calldata gasInfo
  ) internal {
  ...
  function estimateGasFee(
    ...
    GasEstimationType gasEstimationType =
      GasEstimationType(gasInfo.gasEstimationType);

    gasEstimate =
      gasInfo.axelarBaseFee + (executionGasLimit * gasInfo.relativeGasPrice);
    ...
    if (gasEstimationType != GasEstimationType.Default) {
      ...
      gasEstimate +=
        computeL1DataFee(gasEstimationType, payload, gasInfo, l1GasInfo);
    }
  }
  ...
  function optimismEcotoneL1Fee(
    ...
    uint256 weightedGasPrice = 16 * gasInfo.l1FeeScalar
      * l1GasInfo.relativeGasPrice
      + blobBaseFeeScalar * l1GasInfo.relativeBlobBaseFee;
  ...
  function optimismBedrockL1Fee(
    ...
    return
      (l1GasInfo.relativeGasPrice * txSize * gasInfo.l1FeeScalar) / precision;
  }
  ...
  function arbitrumL1Fee(
    ...
    return (
      l1GasInfo.relativeGasPrice * (units + estimationPaddingUnits)
        * (oneInBips + estimationPaddingBasisPoints)
    ) / oneInBips;
  }
  ...
  function _l1TxSize(
    bytes calldata payload
  ) private pure returns (uint256 txSize) {

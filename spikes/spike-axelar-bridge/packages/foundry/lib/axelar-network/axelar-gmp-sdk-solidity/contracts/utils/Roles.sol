...
contract Roles is RolesBase, IRoles {
  ...
  function getAccountRoles(
    address account
  ) public view returns (uint256 accountRoles) {
   ...
   */
  function getProposedRoles(
    address fromAccount,
    address toAccount
  ) public view returns (uint256 proposedRoles_) {
  ...
  function proposeRoles(
    ...
    if (toAccount == address(0) || toAccount == msg.sender) {
      revert InvalidProposedAccount(toAccount);
    }

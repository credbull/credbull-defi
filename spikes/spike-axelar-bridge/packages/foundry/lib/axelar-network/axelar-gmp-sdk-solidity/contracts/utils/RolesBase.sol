...
contract RolesBase is IRolesBase {
   ...
   */
  modifier withEveryRole(
    uint8[] memory roles
  ) {
    uint256 accountRoles = _toAccountRoles(roles);
    if (!_hasAllTheRoles(_getRoles(msg.sender), accountRoles)) {
   ...
   */
  modifier withAnyRole(
    uint8[] memory roles
  ) {
    uint256 accountRoles = _toAccountRoles(roles);
    if (!_hasAnyOfRoles(_getRoles(msg.sender), accountRoles)) {
  ...
  function _rolesKey(
    address account
  ) internal view virtual returns (bytes32 key) {
  ...
  function _getRoles(
    address account
  ) internal view returns (uint256 accountRoles) {
  ...
  function _proposalKey(
    ...
    return
      keccak256(abi.encodePacked(PROPOSE_ROLES_PREFIX, fromAccount, toAccount));
  }
   ...
   */
  function _getProposedRoles(
    address fromAccount,
    address toAccount
  ) internal view returns (uint256 proposedRoles_) {
   ...
   */
  function _hasAllTheRoles(
    uint256 hasAccountRoles,
    uint256 mustHaveAccountRoles
  ) internal pure returns (bool) {
  ...
  function _hasAnyOfRoles(
    uint256 hasAccountRoles,
    uint256 mustHaveAnyAccountRoles
  ) internal pure returns (bool) {
  ...
  function _proposeAccountRoles(
    ...
    if (!_hasAllTheRoles(_getRoles(fromAccount), accountRoles)) {
      revert MissingAllRoles(fromAccount, accountRoles);
    }

...
import { IInterchainAddressTracker } from
  "../interfaces/IInterchainAddressTracker.sol";
...
contract InterchainAddressTracker is IInterchainAddressTracker {
  bytes32 internal constant PREFIX_ADDRESS_MAPPING =
    keccak256("interchain-address-tracker-address-mapping");
  ...
  bytes32 internal constant _CHAIN_NAME_SLOT =
    0x0e2c162a1f4b5cff9fdbd6b34678a9bcb9898a0b9fbca695b112d61688d8b2ac;

  function _setChainName(
  ...
  function trustedAddress(
    string memory chain
  ) public view returns (string memory trustedAddress_) {
   ...
   */
  function trustedAddressHash(
    string memory chain
  ) public view returns (bytes32 trustedAddressHash_) {
   ...
   */
  function _getTrustedAddressSlot(
    string memory chain
   ...
   */
  function _getTrustedAddressHashSlot(
    string memory chain
  ) internal pure returns (bytes32 slot) {
   ...
   */
  function _removeTrustedAddress(
    string memory chain
  ) internal {
    if (bytes(chain).length == 0) revert ZeroStringLength();

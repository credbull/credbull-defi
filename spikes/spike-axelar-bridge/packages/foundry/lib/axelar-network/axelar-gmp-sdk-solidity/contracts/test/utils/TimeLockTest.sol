...
contract TimeLockTest is TimeLock {
  ...
  constructor(
    uint256 _minimumTimeDelay
  ) TimeLock(_minimumTimeDelay) {
    if (PREFIX_TIME_LOCK != keccak256("time-lock")) {
      revert("invalid time-lock slot");
    }
  }

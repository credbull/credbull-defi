...
import { IAxelarExpressExecutable } from
  "../interfaces/IAxelarExpressExecutable.sol";

abstract contract ExpressExecutorTracker is IAxelarExpressExecutable {
  bytes32 internal constant PREFIX_EXPRESS_EXECUTE =
    keccak256("express-execute");
  bytes32 internal constant PREFIX_EXPRESS_EXECUTE_WITH_TOKEN =
    keccak256("express-execute-with-token");
  ...
  function getExpressExecutor(
    ...
    bytes32 slot =
      _expressExecuteSlot(commandId, sourceChain, sourceAddress, payloadHash);
  ...
  function getExpressExecutorWithToken(
    ...
    bytes32 slot = _expressExecuteWithTokenSlot(

 ...
 */
contract InterchainMultisig is
  Caller,
  ...
  IInterchainMultisig
{
  ...
  function executeCalls(
    ...
    for (uint256 i; i < callsLength; ++i) {
      ...
      if (
        keccak256(bytes(call.chainName)) == chainNameHash
          && call.executor == address(this)
      ) {
   ...
   */
  function _interchainMultisigStorage()
    private
    pure
    returns (InterchainMultisigStorage storage slot)
  {
    assembly {

 ...
 */
contract InterchainGovernance is
  AxelarExecutable,
  TimeLock,
  Caller,
  IInterchainGovernance
{
  ...
  constructor(
    ...
    if (
      bytes(governanceChain_).length == 0
        || bytes(governanceAddress_).length == 0
    ) {
  ...
  function executeProposal(
    ...
    emit ProposalExecuted(
      proposalHash, target, callData, nativeValue, block.timestamp
    );
  ...
  function _execute(
    ...
    (
      ...
      uint256 nativeValue,
      uint256 eta
    ) = abi.decode(payload, (uint256, address, bytes, uint256, uint256));

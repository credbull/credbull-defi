...
const getWeightedSignersSet = (signerAddresses, weights, threshold) => {
  const signersWithWeights = signerAddresses.map((address, i) => ({
    address,
    weight: weights[i],
  }));
  const sortedSignersWithWeights = sortBy(signersWithWeights, (signer) =>
    signer.address.toLowerCase()
  );
  const sortedAddresses = sortedSignersWithWeights.map(
    ({ address }) => address
  );
  ...
  return defaultAbiCoder.encode(
    ...
    [sortedAddresses, sortedWeights, threshold]
  );
};

const WEIGHTED_SIGNERS_TYPE =
...
const encodeWeightedSignersMessage = (
  data,
  domainSeparator,
  weightedSignerHash
) => {
  return arrayify(
    domainSeparator +
      weightedSignerHash.slice(2) +
      keccak256(arrayify(data)).slice(2)
  );
};

const encodeMessageHash = (data, domainSeparator, weightedSignerHash) => {
  return hashMessage(
    encodeWeightedSignersMessage(data, domainSeparator, weightedSignerHash)
  );
};

const getWeightedSignersProof = async (
  data,
  domainSeparator,
  weightedSigners,
  wallets
  ...
  const message = encodeWeightedSignersMessage(
    data,
    domainSeparator,
    weightedSignerHash
  );
...
const encodeInterchainCallsBatch = (batchId, calls) =>
  defaultAbiCoder.encode(
    ["bytes32", "tuple(string, address, address, bytes, uint256)[]"],
    [batchId, calls]
  );

...
import { IProduct } from "@credbull/contracts/interfaces/IProduct.sol";
 ...
 */
contract ProductScenarioTest is Test {
    ...
    // NOTE (JL,2024-09-04): Strictly 'share' is an implementation detail, so the spec is in terms of a ERC4626 vault.
    IERC20 internal share;

    // NOTE (JL,2024-09-04): Having a single interface as the product is, I think untenable. But its a decent starting point.
    IProduct internal _product;

    // NOTE (JL,2024-09-04): We start building against the most product-like item, and will refactor it out later.
    TimelockInterestVault private vault;

    function product() internal virtual returns (IProduct) {
        // NOTE (JL,2024-09-04): Basic starting parameters.
        vault = new TimelockInterestVault(OWNER, asset, INTEREST_RATE_PERCENTAGE, FREQUENCY, TENOR);
    ...
    // NOTE (JL,2024-09-02): IProduct is inherently IRC4626 because it operates on Assets & Shares.

    function setUp() public {
        uint256 tokenSupply = 10_000_000 ether;

        vm.startPrank(OWNER);
        // SimpleToken uses 18 decimals, matching the 'ether' amounts.
        asset = new SimpleToken(tokenSupply);
        vm.stopPrank();
        assertEq(asset.balanceOf(OWNER), tokenSupply, "owner should start with total supply");

        uint256 userTokenAmount = 100_000 ether;
        vm.startPrank(OWNER);
        asset.transfer(ALICE, userTokenAmount);
        asset.transfer(BOB, userTokenAmount);

        product();
        asset.transfer(address(_product), userTokenAmount * 2);

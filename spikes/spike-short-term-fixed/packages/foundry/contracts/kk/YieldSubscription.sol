...
contract YieldSubscription is IProduct {
    ...
    function getTermPeriod() public view override returns (uint256) {
        ...
        if (eligibleAmount > 0) {
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                if (window < currentWindow) {
                    uint256 balance = userReserve[user][window] + calcInterestForDepositTimePeriod(user, window);
                    if (shares > balance) {
                        shares -= balance;
    ...
    function calculateEligibleAmount(address user) public view returns (uint256 amountToTransfer) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            amountToTransfer += (userReserve[user][window] + calcInterestForDepositTimePeriod(user, window));
        }
    }

    function calcTotalInterest(address user) public view returns (uint256) {
        uint256 currentWindow = getCurrentTimePeriodsElapsed();
        uint256 length = userWindows[user].length();
        ...
        for (uint256 i = 0; i < length; i++) {
            uint256 window = userWindows[user].at(i);
            if (window < currentWindow) {
                interestEarned += calcInterestForDepositTimePeriod(user, window);
            }
        }
        return interestEarned;
    }

    function calcTotalDeposits(address user) public view returns (uint256) {
        uint256 currentWindow = getCurrentTimePeriodsElapsed();
        uint256 length = userWindows[user].length();
    ...
    function calcInterestForDepositTimePeriod(address user, uint256 window) public view returns (uint256) {
        ...
        if (noOfWindowsPassed > maturityPeriod) {
            ...
            return (
                interestEarnedBeofreRollOver
                    + (
                        (userDeposit + interestEarnedBeofreRollOver) * yieldPerWindowRollOver()
                            * (noOfWindowsPassed - maturityPeriod)
                    ) / 1e20
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (amount > userDeposit) {
            return calcInterestForDepositTimePeriod(user, window);
        }
        userDeposit = amount;
        uint256 noOfWindowsPassed = getCurrentTimePeriodsElapsed() - window;
    ...
    function balanceWithInterest(address user) public view returns (uint256) {
        return calcTotalDeposits(user) + calcTotalInterest(user);
    }

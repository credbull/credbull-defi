...
contract ShortTermFixedYieldVault is ERC721, Ownable2Step, Pausable {
    ...
    function deposit(uint256 amount) public whenNotPaused isVaultOpen returns (uint256 tokenId) {
        ...
        if (tokenId != 0 && depositInfos[tokenId].timePeriodsFromOpen == currentTimePeriodsElapsed) {
            ...
            DepositInfo memory newDeposit =
                DepositInfo({ principal: amount, timePeriodsFromOpen: currentTimePeriodsElapsed });
     ...
     */
    function getDepositLockTimePeriods(uint256 depositTimePeriodsFromOpen)
        public
        view
        returns (uint256 curDepositLockTimePeriods)
    {
     ...
     */
    function getTimePeriodsInCurrentTermStart(uint256 depositTimePeriodsFromOpen) public view returns (uint256) {
     ...
     */
    function calculateCompoundingAmount(uint256 principal, uint256 apy, uint256 noOfTerms, uint256 termPeriods)
        public
        pure
        returns (uint256)
    {
     ...
     */
    function calculateAmountWithFixedInterest(uint256 principal, uint256 apy, uint256 noOfPeriods)
        public
        pure
        returns (uint256)
    {
     ...
     * @return accumulatedAmount amount
     */
    function calculateAccumulatedAmount(uint256 tokenId) public view returns (uint256) {
    ...
    function getDepositInfo(uint256 tokenId)
        external
        view
        returns (uint256 periodsUntilTermEnd, uint256 currentYield, uint256 accumulatedAmount)
    {
        DepositInfo memory depositInfo = depositInfos[tokenId];
    ...
    function _calculateLastTermWithdrawal(uint256 tokenId) internal view returns (uint256) {
        DepositInfo memory depositInfo = depositInfos[tokenId];
        ...
        if (noOfTermsElapsed > 0) {
            withdrawalAmount =
                calculateCompoundingAmount(withdrawalAmount, FIXED_APY, noOfTermsElapsed, LOCK_TIME_PERIODS);
        }

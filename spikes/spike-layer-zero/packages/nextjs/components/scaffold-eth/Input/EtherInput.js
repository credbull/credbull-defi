...
exports.EtherInput = void 0;
const react_1 = require("react");
const outline_1 = require("@heroicons/react/24/outline");
const scaffold_eth_1 = require("~~/components/scaffold-eth");
const useDisplayUsdMode_1 = require("~~/hooks/scaffold-eth/useDisplayUsdMode");
...
function etherValueToDisplayValue(usdMode, etherValue, nativeCurrencyPrice) {
    if (usdMode && nativeCurrencyPrice) {
        ...
        if (Number.isNaN(parsedEthValue)) {
            ...
            return (Math.round(parsedEthValue * nativeCurrencyPrice * 10 ** MAX_DECIMALS_USD) /
                10 ** MAX_DECIMALS_USD).toString();
        }
    }
    else {
        return etherValue;
 ...
 */
const EtherInput = ({ value, name, placeholder, onChange, disabled, usdMode, }) => {
    const [transitoryDisplayValue, setTransitoryDisplayValue] = (0, react_1.useState)();
    const nativeCurrencyPrice = (0, store_1.useGlobalState)(state => state.nativeCurrency.price);
    const isNativeCurrencyPriceFetching = (0, store_1.useGlobalState)(state => state.nativeCurrency.isFetching);
    const { displayUsdMode, toggleDisplayUsdMode } = (0, useDisplayUsdMode_1.useDisplayUsdMode)({ defaultUsdMode: usdMode });
    ...
    const handleChangeNumber = (newValue) => {
        if (newValue && !scaffold_eth_1.SIGNED_NUMBER_REGEX.test(newValue)) {
            return;
        }
    ...
    return (<scaffold_eth_1.InputBase name={name} value={displayValue} placeholder={placeholder} onChange={handleChangeNumber} disabled={disabled} prefix={<span className="pl-4 -mr-2 text-accent self-center">{displayUsdMode ? "$" : "Ξ"}</span>} suffix={<div className={`${nativeCurrencyPrice > 0
                ? ""
                : "tooltip tooltip-secondary before:content-[attr(data-tip)] before:right-[-10px] before:left-auto before:transform-none"}`} data-tip={isNativeCurrencyPriceFetching ? "Fetching price" : "Unable to fetch price"}>
          <button className="btn btn-primary h-[2.2rem] min-h-[2.2rem]" onClick={toggleDisplayUsdMode} disabled={!displayUsdMode && !nativeCurrencyPrice}>
            <outline_1.ArrowsRightLeftIcon className="h-3 w-3 cursor-pointer" aria-hidden="true"/>
          </button>
        </div>}/>);
};
exports.EtherInput = EtherInput;

...
abstract contract TimelockAsyncUnlock is Initializable, ITimelockAsyncUnlock, ContextUpgradeable {
        ...
        returns (uint256[] memory depositPeriods, uint256[] memory amounts)
    {
        ...
        if (requestId > currentPeriod()) {
            revert TimelockAsyncUnlock__UnlockBeforeUnlockPeriod(_msgSender(), owner, currentPeriod(), requestId);
        }
     ...
     */
    function _unlock(address owner, uint256 depositPeriod, uint256 requestId, uint256 amountToUnlock)
        internal
        virtual
    {
        ...
        virtual
    {
        ...
        uint256 unlockAmountByOwner =
            depositPeriodAmountCache.contains(depositPeriod) ? depositPeriodAmountCache.get(depositPeriod) : 0;

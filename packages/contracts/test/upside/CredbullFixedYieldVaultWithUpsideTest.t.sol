...
import { CredbullFixedYieldVaultWithUpside } from "../../src/upside/CredbullFixedYieldVaultWithUpside.sol";
...
contract CredbullFixedYieldVaultWithUpsideTest is Test {
    ...
    function test__UpsideVault__DepositAssetsAndGetShares() public {
        ...
        assertEq(
            ...
            depositAmount.mulDiv(REQUIRED_COLLATERAL_PERCENTAGE, 100),
    ...
    function test__UpsideVault__RedeemAssetAndBurnShares() public {
        ...
        MockStablecoin(address(vaultParams.asset)).mint(vaultParams.custodian, 100 ether + 1);
    ...
    function test__UpsideVault__WithdrawAssetAndBurnShares() public {
        ...
        vm.startPrank(alice);
        vault.withdraw(depositAmount + 100 ether, alice, alice);
        vm.stopPrank();

        assertEq(vaultParams.token.balanceOf(alice), 200 ether, "Alice should now have the Tokens");
        assertEq(vaultParams.asset.balanceOf(alice), 1100 ether, "Alice should receive finalBalance with 10% yeild");
    }

...
import { DeployAggregateToken } from "@test/test/partners/plume/DeployAggregateToken.s.sol";
...
contract AggregateTokenTest is LiquidContinuousMultiTokenVaultTestBase {
    ...
    function setUp() public override {
        ...
        aggregateToken = deployAggToken.run(_vaultAuth.owner, address(_asset));

        vm.startPrank(_vaultAuth.owner);
        aggregateToken.addComponentToken(IComponentToken(address(_liquidVault))); // list liquidstone on aggToken
        aggregateToken.grantRole(aggregateToken.MANAGER_ROLE(), _vaultAuth.operator); // grant manager role
        vm.stopPrank();
    }
    ...
    function test__AggregateTokenTest__BuyComponentToken() public {
        ...
        _depositAssetsAggregateToken(_alice, depositAmount);
    ...
    function test__AggregateTokenTest__SellComponentToken() public {
        ...
        _liquidVerifier._warpToPeriodLV(_liquidVault, testParams.depositPeriod);

        _depositAssetsAggregateToken(_alice, testParams.principal);
        ...
        _liquidVerifier._warpToPeriodLV(_liquidVault, testParams.redeemPeriod - _liquidVault.noticePeriod());

        // Call requestSellComponentToken
        ...
        uint256 expectedAmount =
            testParams.principal + _liquidVerifier._expectedReturns(_toIVault(_liquidVault), testParams);

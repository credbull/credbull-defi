...
contract TestToken is ERC20, ERC20Permit, ERC20Burnable, ERC20Capped, AccessControl {
    ...
    constructor(address _owner, address _minter, uint256 _maxSupply)
        ERC20("TestTokenV1", "TestTokenV1")
        ERC20Permit("TestTokenV1")
        ERC20Capped(_maxSupply)
    {
        if (_owner == address(0)) {
            revert TestToken__InvalidOwnerAddress();
        }

        if (_minter == address(0)) {
            revert TestToken__InvalidMinterAddress();
        }

...
describe('Login', () => {
  ...
  it('should successfully sign up', () => {
    ...
    cy.get('input#mantine-r0.my-Input-input.my-TextInput-input.my-1g14e8k')
      .click()
      .type(validEmail)
      .invoke('val')
      .should('eq', validEmail);
    cy.get('input#mantine-r1.my-13e8zn2.my-PasswordInput-innerInput')
      .type(validPassword)
    ...
    cy.intercept(
      'POST',
      `${NEXT_PUBLIC_SUPABASE_URL}/auth/v1/signup?redirect_to=http%3A%2F%2Flocalhost%3A3000%2Fcode%2Fcallback/auth/v1/signup?redirect_to=http%3A%2F%2Flocalhost%3A3000%2Fcode%2Fcallback`,
    ).as('signUpApi');

    cy.get('button.my-UnstyledButton-root.my-Button-root.my-t4tusc').click(); // Click on the button
    // .should('have.attr', 'data-button', 'true');
  ...
  it('should fail sign in with inValid credentials', () => {
    ...
    cy.intercept('POST', `${NEXT_PUBLIC_SUPABASE_URL}/auth/v1/token?grant_type=password`, (req) => {
      req.reply({
        statusCode: 400,
        body: invalidCredentialsErrorBody,
      });
    }).as('failedRequest');

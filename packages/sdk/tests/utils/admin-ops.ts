...
import { CredbullFixedYieldVault, CredbullFixedYieldVaultWithUpside, MockStablecoin__factory } from '@credbull/contracts';
...
export async function toggleMaturityCheck(vault: CredbullFixedYieldVault | CredbullFixedYieldVaultWithUpside, value: boolean) {
    const adminSigner = await getAdminSigner();
    await vault.connect(adminSigner).toggleMaturityCheck(value);
}

export async function toggleWindowCheck(vault: CredbullFixedYieldVault | CredbullFixedYieldVaultWithUpside, value: boolean) {
    const adminSigner = await getAdminSigner();

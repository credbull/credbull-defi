...
test.beforeAll(async () => {
  const { access_token: userAToken, user_id: userAId } = await login(process.env.USER_A_EMAIL || '', process.env.USER_A_PASSWORD || '');
  const { access_token: userBToken, user_id: userBId } = await login(process.env.USER_B_EMAIL || '', process.env.USER_B_PASSWORD || '');
...
test.describe('Multi user Interaction', async () => {

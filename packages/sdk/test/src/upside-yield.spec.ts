...
import { TRASH_ADDRESS, __mockMint, __mockMintToken } from './utils/contracts';
...
test.describe.skip('Upside Yield', async () => {
  ...
  test('10% + 20% upside', async () => {
    ...
    await test.step('Create upside vault', async () => {
      const { privateKey: treasuryPk, address: treasury } = generateAddress('treasury_upside');
      const { privateKey: activityRewardPk, address: activityReward } = generateAddress('activity_reward_upside');
      await createFixedYieldWithUpsideVault(config, undefined, treasury, activityReward, 200);
    });
    ...
    await test.step('Empty custodian', async () => {
      ...
      await usdc.connect(alice.testSigner.getDelegate()).transfer(TRASH_ADDRESS, userABalance);
      await usdc.connect(bob.testSigner.getDelegate()).transfer(TRASH_ADDRESS, userBBalance);
    });
    ...
    await test.step('Redeem through SDK', async () => {
      ...
      await __mockMint(custodian, BigNumber.from('100000000'), vault, alice.testSigner.getDelegate());
  ...
  test('10% + 30% upside', async () => {
    ...
    await test.step('Create upside vault', async () => {
      const { privateKey: treasuryPk, address: treasury } = generateAddress('treasury_upside');
      const { privateKey: activityRewardPk, address: activityReward } = generateAddress('activity_reward_upside');
      await createFixedYieldWithUpsideVault(config, undefined, treasury, activityReward, 300);
    });
